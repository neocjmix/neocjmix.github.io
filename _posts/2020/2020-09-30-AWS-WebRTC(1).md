...
또한 일부 모바일 기기에서 `<video />` 태그를 전체화면 모드가 아닌 상태에서 제대로 재생하기 위해서는 `playsinline` 속성을
추가해주어야 한다.
...
결론적으로 서비스를 완성하기 위해서는 제대로 임의의 사용자가 접속해서 이용할 수 있는 public ip와 SSL로 인증된 도메인을 갖춘 환경을 구축해야 한다.
이러한 문제를 해결하기 위해 AWS와 같은 클라우드 서비스를 이용하여 websocket 연결을 구현할 필요가 있다.
또한 현재 시그널링 서버 구현으로는 두개 이상의 기기가 접속하면 제대로 연결을 맺어줄 수 없으므로, 여러 사용자가 접속해서 원하는 상대방을 찾을 수 있는 방법을 구현해야 한다. 

<u>하지만 일단은 어떻게든 로컬에서 서버 구현을 https방식으로 바꾸기만 하면 현상태에서도 모바일 기기에서 제대로 작동한다.</u>
이 내용을 포함하여 위에 나열한 과제들은 다음 포스팅에서 다룰 예정이다. 
